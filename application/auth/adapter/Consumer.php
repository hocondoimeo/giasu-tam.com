 ...
 */
class My_Auth_Adapter_Consumer extends Zend_OpenId_Consumer
{
    /**
        ...
        $extensions=null, Zend_Controller_Response_Abstract $response = null)
    {
        ...
        $rt = explode('openid_identifier', Zend_OpenId::absoluteUrl($returnTo));
        $params['openid.return_to'] = rtrim($rt[0], '/');
        ...
        if ($version >= 2.0) {
        	$root = explode('openid_identifier', $root);
            $params['openid.realm'] = rtrim($root[0], '/');
        } else {
